...
export function StatusIconify({ status, size, sx }: StatusIconifyProps) {
    ...
    const theme = useTheme();
    const default_grey = theme.palette.grey[500];
    ...
    function getStatusIconInfo(type: 'icon' | 'color') {
        switch (status) {
            ...
            case 'closed':
                return type === 'icon' ? 'eva:minus-outline' : default_grey;
            case 'low_complexity':
                ...
                return type === 'icon' ? 'line-md:chevron-up' : default_grey;
            case 'medium_complexity':
                ...
                return type === 'icon' ? 'line-md:chevron-double-up' : default_grey;
            case 'high_complexity':
                ...
                return type === 'icon' ? 'line-md:chevron-triple-up' : default_grey;
            case 'low_priority':
                return type === 'icon' ? 'iconoir:priority-down' : theme.palette.primary.main;
            case 'medium_priority':
                return type === 'icon' ? 'iconoir:priority-medium' : default_grey;
            case 'high_priority':
                return type === 'icon' ? 'iconoir:priority-high' : theme.palette.error.main;
            case 'private':
                return type === 'icon' ? 'eva:lock-outline' : default_grey;
            case 'public':
                return type === 'icon' ? 'eva:globe-outline' : default_grey;
            case 'moderator':
                return type === 'icon' ? 'eos-icons:admin-outlined' : default_grey;
            case 'user':
                return type === 'icon' ? 'mdi:user' : default_grey;
            default:
                return type === 'icon' ? 'eva:clock-outline' : theme.palette.warning.main;
        };
    };
